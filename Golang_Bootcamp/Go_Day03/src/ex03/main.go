...
func handlerLogin(c *gin.Context,  htmlTemplate string, places []Source, totalRecord int) {
	tmpl := template.Must(template.New("htmlTemplate").Parse(htmlTemplate))
	pageSize := 10
	// pageNum := c.Param("pageNum")
	pageNum := c.DefaultQuery("page", "")
...
func main() {
	...
	htmlTemplateBytes2, err2 := ioutil.ReadFile("auth.html")
	if err2 != nil {
		fmt.Println("Ошибка при чтении файла:", err)
		return
	}

	htmlTemplate2 := string(htmlTemplateBytes2)
	...
	router.GET("/login", func(c *gin.Context) {
		handlerLogin(c, htmlTemplate2, places, totalRecord)
	})
