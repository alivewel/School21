...
import (
	...
	"transmitter/pkg/db"
	ar "transmitter/pkg/add_response"
)
...
func main() {
	...
	if (len(responseArray) > 2) {
		DB := db.Init()
		response := responseArray[len(responseArray) - 1]
		stdDev := rand.Float64() * k
		if response.Frequency > stdDev || response.Frequency < -stdDev {
			log.Printf("Anomal Frequency: %f", response.Frequency)
			ar.AddResponse(response, DB)
			// аномальные частоты заносим в БД
		}
	}
}
